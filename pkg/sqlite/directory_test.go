...
func TestQuerierForDirectory(t *testing.T) {
	...
	etcdBundleByReplaces, _, err := store.GetBundleThatReplaces(context.TODO(), "etcdoperator.v0.9.0", "etcd", "alpha")
	fmt.Printf("My bundle: %s\n", etcdBundleByReplaces)
	require.NoError(t, err)
	...
	etcdBundleByProvides, _, entry, err := store.GetBundleThatProvides(context.TODO(), "etcd.database.coreos.com", "v1beta2", "EtcdCluster")
	require.NoError(t, err)
