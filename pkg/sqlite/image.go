...
func (i *ImageLoader) LoadBundleFunc() error {
	...
	for _, f := range files {
		...
		if f.Name() == DependenciesFile {
			err = decoder.Decode(&dependenciesFile)
			if err == nil {
	...
	err = i.loadManifests(manifests, annotationsFile, dependenciesFile)
	if err != nil {
		return err
	}
...
func (i *ImageLoader) loadManifests(manifests string, annotationsFile *registry.AnnotationsFile, dependenciesFile *registry.DependenciesFile) error {
